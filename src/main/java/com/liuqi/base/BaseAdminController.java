...
public abstract class BaseAdminController<T extends BaseModel, TDO extends T> extends BaseController {
    ...
    public ReturnResponse update(@CurAdminId Long adminId, @Validated(AdminValid.class) TDO t, BindingResult bindingResult, ModelMap modelMap, HttpServletRequest request, HttpServletResponse response) {
        ...
        if (updateLogger()) {
            opeLogger.set(getLogger(adminId, LoggerModelDto.TYPE_UPDATE, "更新", "修改前：" + JSONObject.toJSONString(t) + "修改后：" + JSONObject.toJSONString(oldM)));
        }
    ...
    protected  boolean checkPwd(Long adminId,String pwd){
        boolean check=false;
        String key = KeyConstant.KEY_ADMIN_AUTH_ERROR + adminId;
        Integer times = repository.getInteger(key);
        if (times >= 3) {
            throw new BusinessException("密码错误3次，请10分钟后再试");
        }
        UserAdminModel admin=adminService.getSecret(adminId);
        check= ShiroPasswdUtil.getAdminPwd(pwd).equals(admin.getPwd());
        if(!check){
            times++;
            if (times == 1) {
                repository.set(key, times, 10L, TimeUnit.MINUTES);
            } else {
                repository.incrOne(key);
            }
        }else{
            repository.del(key);
        }
