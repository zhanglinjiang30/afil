package com.liuqi.third.email;

import com.aliyuncs.DefaultAcsClient;
import com.aliyuncs.IAcsClient;
import com.aliyuncs.dm.model.v20151123.SingleSendMailRequest;
import com.aliyuncs.dm.model.v20151123.SingleSendMailResponse;
import com.aliyuncs.profile.DefaultProfile;
import com.aliyuncs.profile.IClientProfile;
import com.liuqi.business.model.SmsConfigModel;
import com.liuqi.business.model.SmsConfigModelDto;
import com.liuqi.business.service.SmsConfigService;
import lombok.extern.slf4j.Slf4j;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Component;

/**
 * 阿里邮件
 */
@Component
@Slf4j
public class AliEmailSender {

    public static final String ACCESSKEY="LTAIcEGDd7mVQJNd";
    public static final String ACCESSSECRET="XYEXcVi3Sk6YIacHuBdevvHmzzrwny";

    @Autowired
    private SmsConfigService smsConfigService;

    public void sendSimpleMail(String to,String subject,String content,String sign) {
        SmsConfigModelDto config=smsConfigService.getConfig();
        // 如果是除杭州region外的其它region（如新加坡、澳洲Region），需要将下面的"cn-hangzhou"替换为"ap-southeast-1"、或"ap-southeast-2"。
        IClientProfile profile = DefaultProfile.getProfile(config.getEmailRegionId(), config.getEmailAccessKeyId(), config.getEmailSecret());
        // 如果是除杭州region外的其它region（如新加坡region）， 需要做如下处理
        /*try {
            DefaultProfile.addEndpoint("dm.ap-southeast-1.aliyuncs.com", "ap-southeast-1", "Dm",  "dm.ap-southeast-1.aliyuncs.com");
        } catch (ClientException e) {
        e.printStackTrace();
        }*/
        IAcsClient client = new DefaultAcsClient(profile);
        SingleSendMailRequest request = new SingleSendMailRequest();
        try {
            //request.setVersion("2017-06-22");// 如果是除杭州region外的其它region（如新加坡region）,必须指定为2017-06-22
            request.setAccountName(config.getEmailAccountName());//控制台创建的发信地址
            request.setFromAlias(sign);//发信人昵称
            request.setAddressType(1);
            request.setTagName(config.getEmailTag());//控制台创建的标签
            request.setReplyToAddress(true);
            request.setToAddress(to);
            request.setSubject(subject);
            request.setHtmlBody(content);
            SingleSendMailResponse httpResponse = client.getAcsResponse(request);
            log.info("邮件发送成功--》"+content);
        } catch (Exception e) {
            e.printStackTrace();
            log.info("邮件发送异常--》"+to+"-->"+content);
        }
    }
}
