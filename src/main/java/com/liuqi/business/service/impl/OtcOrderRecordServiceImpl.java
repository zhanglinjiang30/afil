...
public class OtcOrderRecordServiceImpl extends BaseServiceImpl<OtcOrderRecordModel,OtcOrderRecordModelDto> implements OtcOrderRecordService{
	...
	protected void doMode(OtcOrderRecordModelDto dto) {
		...
		String buyName = addressHoldingRecordService.getNameByAddressId(dto.getBuyUserId());
		String sellName = addressHoldingRecordService.getNameByAddressId(dto.getSellUserId());
		String appealName = addressHoldingRecordService.getNameByAddressId(dto.getAppealUser());

		dto.setBuyUserName(buyName);
		dto.setSellUserName(sellName);
		dto.setAppealUserName(appealName);
		dto.setBuyOtcName(buyName);
		dto.setSellOtcName(sellName);
		dto.setAppealOtcName(appealName);
	}
	...
	public Long createRecord(Long userId, OtcOrderModelDto order, BigDecimal quantity) {
		...
		//日志
		String opeName = addressHoldingRecordService.getNameByAddressId(userId);
	...
	public void take(Long userId, Long recordId) {
		...
		if (!record.getStatus().equals(OtcOrderRecordStatusEnum.WAIT.getCode())) {
			throw new BusinessException(MessageSourceHolder.getMessage("message72"));
		}
		OtcOrderModel order = otcOrderService.getById(record.getOrderId());
		if (!userId.equals(order.getUserId())) {
			throw new BusinessException(MessageSourceHolder.getMessage("message73"));
		}
	...
	@Override
	@Transactional
	public void pay(Long userId, Long recordId) {
		OtcOrderRecordModel record = this.getById(recordId);
		if (record == null) {
			throw new BusinessException(MessageSourceHolder.getMessage("message71"));
		}
		if (!record.getStatus().equals(OtcOrderRecordStatusEnum.WAITPAY.getCode())) {
			throw new BusinessException(MessageSourceHolder.getMessage("message74"));
		}
		if (!record.getBuyUserId().equals(userId)) {
			throw new BusinessException(MessageSourceHolder.getMessage("message75"));
		}
	...
	public void gathering(Long userId, Long recordId) {
		...
		if (!record.getStatus().equals(OtcOrderRecordStatusEnum.WAITGATHERING.getCode())) {
			throw new BusinessException(MessageSourceHolder.getMessage("message76"));
		}
		if (!record.getSellUserId().equals(userId)) {
			throw new BusinessException(MessageSourceHolder.getMessage("message77"));
		}
	...
	private void gathering(OtcOrderRecordModel record, String opeName, String remark) {
		...
		UserOtcWalletModel sellWallet =userWalletService.modifyWallet(record.getSellUserId(), record.getCurrencyId(),sellChangeUsing,sellChangeFreeze);
//		userWalletLogService.addLog(record.getSellUserId(), record.getCurrencyId(), sellChangeFreeze, WalletLogTypeEnum.OTC.getCode(), record.getId() ,"OTC交易卖扣除",  sellWallet);

		//买家获取币种
		BigDecimal buyChangeUsing= record.getQuantity();
		BigDecimal buyChangeFreeze=BigDecimal.ZERO;
		UserOtcWalletModel buyWallet =userWalletService.modifyWallet(record.getBuyUserId(), record.getCurrencyId(),buyChangeUsing,buyChangeFreeze);
		userWalletLogService.addLog(record.getBuyUserId(), record.getCurrencyId(), buyChangeUsing, WalletLogTypeEnum.OTC.getCode(), record.getId() ,"OTC交易买获取",  buyWallet);
	...
	public void cancel(Long userId, Long recordId) {
		...
		if (!record.getStatus().equals(OtcOrderRecordStatusEnum.WAIT.getCode())
			...
			throw new BusinessException(MessageSourceHolder.getMessage("message78"));
		}

		if (!record.getStatus().equals(OtcOrderRecordStatusEnum.WAIT.getCode())
				&& !record.getSellUserId().equals(userId)) {
			throw new BusinessException(MessageSourceHolder.getMessage("message79"));
		}

		//处理敲
		String opeName = addressHoldingRecordService.getNameByAddressId(userId);
		this.cancel(record, opeName, "用户取消订单");
	...
	private void cancel(OtcOrderRecordModel record, String opeName, String remark) {
		//返还主单交易量
		OtcOrderModel order = otcOrderService.getById(record.getOrderId());
		...
		if (record.getType().equals(BuySellEnum.BUY.getCode())) {
			...
			UserOtcWalletModel wallet =userWalletService.modifyWallet(record.getSellUserId(), record.getCurrencyId(),changeUsing,changeFreeze);
			userWalletLogService.addLog(record.getSellUserId(), record.getCurrencyId(), changeUsing, WalletLogTypeEnum.OTC.getCode(), record.getId() , "OTC交易取消返还",  wallet);
		}
	...
	public void appealSuccess(Long userId, Long recordId, String remark) {
		...
		if (!record.getStatus().equals(OtcOrderRecordStatusEnum.APPEAL.getCode())) {
			throw new BusinessException(MessageSourceHolder.getMessage("message80"));
		}
		...
		if (record.getAppealType().equals(BuySellEnum.BUY.getCode())) {
			...
			//取消订单
			this.cancel(record, opeName,"申诉成功：取消订单");
			record.setSuccessStatus(OtcOrderRecordSuccessStatusEnum.CANCEL.getCode());
		}
		this.update(record);
	}
	...
	public void appealFail(Long userId, Long recordId, String remark) {
		...
		if (record.getAppealType().equals(BuySellEnum.BUY.getCode())) {
			//收款
			this.cancel(record, opeName,"申诉失败：取消订单");
			record.setSuccessStatus(OtcOrderRecordSuccessStatusEnum.CANCEL.getCode());
