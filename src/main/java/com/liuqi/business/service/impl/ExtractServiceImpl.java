...
public class ExtractServiceImpl extends BaseServiceImpl<ExtractModel,ExtractModelDto> implements ExtractService{
	...
	public void extractApply(ExtractModel extractModel,CurrencyConfigModel config) {
		...
		if (StringUtils.equals("USDT", currency.getName())) {
			if (!extractModel.getAddress().startsWith("0x") ||
			  	extractModel.getAddress().length() != 42) {
				throw new BusinessException(MessageSourceHolder.getMessage("message63"));
			}
		}
		...
		if(checkDayMax) {
			...
			if (config.getExtractMaxDay().compareTo(BigDecimal.ZERO)>0 && quantity.compareTo(config.getExtractMaxDay()) > 0) {
				throw new BusinessException(MessageSourceHolder.getMessage("message20") + config.getExtractMaxDay());
			}
		}
		...
		if(using) {
			...
			if (canUsing.compareTo(extractModel.getQuantity()) < 0) {
				throw new BusinessException(MessageSourceHolder.getMessage("message19"));
			}
		}else{
		...
		// 添加交易记录
		transferRecordService.addRecord(TransferTypeEnum.EXTRACT.getCode(), addressRecordService.getById(extractModel.getUserId()).getAddress(),
				toAddress, BigDecimal.ZERO, sucId, sucId, getExtractFee());
	}
	...
	public void doSuccess(ExtractModel model, ExtractSearchDto dto) {
		if (StringUtils.isNotEmpty(dto.getHash())) {
			...
			if (dto.getToAddress().equals(model.getAddress()) && (dto.getQuantity().compareTo(model.getRealQuantity()) == 0)) {
				...
				throw new BusinessException(MessageSourceHolder.getMessage("message64")+"：" + dto.getQuantity()
						+ ","+MessageSourceHolder.getMessage("message65")+"：" + dto.getToAddress());
			}
		}
	}
...
//			this.sendSms(extractRecord);
	...
	protected void doMode(ExtractModelDto dto) {
		...
		if(dto.getReceiveUserId()!=null && dto.getReceiveUserId()>0) {
			dto.setReceiveUserName(addressRecordService.getById(dto.getReceiveUserId()).getAddress());
		}
