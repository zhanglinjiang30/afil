...
public class LockWalletServiceImpl extends BaseServiceImpl<LockWalletModel, LockWalletModelDto> implements LockWalletService {
    ...
    public boolean update(LockWalletModel t) {
        ...
        if (status) {
            ...
            throw new BusinessException(t.getClass().getName() + "-id:" + t.getId() + " error");
        }
    }
    ...
    private void check(LockWalletModel wallet) {
        if (wallet.getLocking().compareTo(BigDecimal.ZERO) < 0) {
            throw new BusinessException("u:" + wallet.getUserId() + ",c:" + wallet.getCurrencyId() + "，"+ MessageSourceHolder.getMessage("message69") +"->" + wallet.getLocking());
        }
        if (wallet.getFreeze().compareTo(BigDecimal.ZERO) < 0) {
            throw new BusinessException("u:" + wallet.getUserId() + ",c:" + wallet.getCurrencyId() + "，"+MessageSourceHolder.getMessage("Message70")+"->" + wallet.getFreeze());
        }
    }
