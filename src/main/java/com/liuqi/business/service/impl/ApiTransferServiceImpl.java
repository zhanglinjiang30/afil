		...
		if (transfer == null) {
			...
			if (total != null) {
				...
				if (total.getTimes() >= config.getDayTimes()) {
					throw new BusinessException(MessageSourceHolder.getMessage("message55")+":" + config.getDayTimes());
				}
				BigDecimal temp = MathUtil.add(quantity, transferDto.getQuantity());
				if (temp.compareTo(config.getDayMaxQuantity()) > 0) {
					throw new BusinessException(MessageSourceHolder.getMessage("message56")+":" + config.getDayMaxQuantity());
				}
			}
