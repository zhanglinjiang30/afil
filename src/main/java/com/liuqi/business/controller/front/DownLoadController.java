...
public class DownLoadController extends BaseFrontController {
    ...
    @ResponseBody
    public JSONObject fileUpload(@RequestParam(value = "file") MultipartFile file, @RequestParam(value = "type", defaultValue = "1") Integer type,
                                 HttpServletRequest request) throws NoLoginException {
        JSONObject back = new JSONObject();
        back.put("code", 0);
        back.put("msg", MessageSourceHolder.getMessage("message5"));
        try {
            // 获取文件名
            String fileName = file.getOriginalFilename();
            String suffix = StringUtils.isNotEmpty(fileName) ? fileName.substring(fileName.lastIndexOf(".") + 1) : "";
            //验证后缀
            FileEnum fileEnum = FileEnum.getCode(type);
            FileEnum.valid(fileEnum, suffix);

            String imageTemp = uploadFileService.uploadFile(file,10);
            JSONObject result = new JSONObject();
            result.put("title", fileName);
            result.put("src", imageTemp);
            back.put("data", result);
            return back;
        } catch (Exception e) {
            back.put("code", 1);
            back.put("msg", e.getMessage());
            e.printStackTrace();
            return back;
        }
    }
