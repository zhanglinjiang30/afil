...
import com.liuqi.sharingjdbc.TableBlockShardingLongAlgorithm;
...
public class ShardingDataSourceConfig {
    ...
    public DataSource dataSource() throws SQLException {
        ...
        shardingRuleConfig.getTableRuleConfigs().add(getBlockTableRuleConfiguration());
        shardingRuleConfig.getTableRuleConfigs().add(getBlockTransactionTableRuleConfiguration());
    ...
    private TableRuleConfiguration getBlockTableRuleConfiguration() {
        TableRuleConfiguration orderTableRuleConfig = new TableRuleConfiguration();
        orderTableRuleConfig.setLogicTable("t_block");
        orderTableRuleConfig.setActualDataNodes("ds_0.t_block_${[0,1,2,3,4]}");
        //单个表的分库
        ...
        //单个表的分表
        orderTableRuleConfig.setTableShardingStrategyConfig(new StandardShardingStrategyConfiguration("height", new TableBlockShardingLongAlgorithm()));
        orderTableRuleConfig.setKeyGeneratorColumnName("id");
        return orderTableRuleConfig;
    }

    private TableRuleConfiguration getBlockTransactionTableRuleConfiguration() {
        TableRuleConfiguration orderTableRuleConfig = new TableRuleConfiguration();
        orderTableRuleConfig.setLogicTable("t_block_transaction");
        orderTableRuleConfig.setActualDataNodes("ds_0.t_block_transaction_${[0,1,2,3,4]}");
        //单个表的分库
        //orderTableRuleConfig.setDatabaseShardingStrategyConfig(new StandardShardingStrategyConfiguration("user_id", DatabaseShardingAlgorithm.class.getName()));
        //单个表的分表
        orderTableRuleConfig.setTableShardingStrategyConfig(new StandardShardingStrategyConfiguration("height", new TableBlockShardingLongAlgorithm()));
